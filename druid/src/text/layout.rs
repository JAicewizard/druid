...
use crate::{Data, Env, FontDescriptor, KeyOrValue, PaintCtx, RenderContext};

/// Some text that can be drawn on the screen.
///
...
pub struct TextLayout {
    ...
    // when set, this will be used to override the size in
    // the font descriptor. This provides an easy way to
    // change only the font size, while still using a `FontDescriptor` in the `Env`.
...
impl TextLayout {
    ...
    /// Given a utf-8 range in the underlying text, return a `Vect` of `Rect`s
    ...
    pub fn rebuild_if_needed(&mut self, factory: &mut PietText, text: &str, env: &Env) {
        ...
        let text_changed = self.layout.as_ref().map(|layout| layout.text()) != Some(text);
